  ...
  grafanaDashboards+:: {
    'coredns.json':
      ...
      local typeRate =
        ...
        .addTarget(prometheus.target('sum(rate(coredns_dns_requests_total{%(clusterLabel)s="$cluster", %(corednsSelector)s,%(instanceLabel)s=~"$instance"}[5m])) by (type)' % $._config, legendFormat='{{type}}'));
      ...
      local zoneRate =
        ...
        .addTarget(prometheus.target('sum(rate(coredns_dns_requests_total{%(clusterLabel)s="$cluster", %(corednsSelector)s,%(instanceLabel)s=~"$instance"}[5m])) by (zone)' % $._config, legendFormat='{{zone}}'));
      ...
      local goroutines =
        ...
        .addTarget(prometheus.target('go_goroutines{%(corednsSelector)s,%(instanceLabel)s=~"$instance"}' % $._config, legendFormat='{{%(clusterLabel)s="$cluster", %(instanceLabel)s}}' % $._config));
